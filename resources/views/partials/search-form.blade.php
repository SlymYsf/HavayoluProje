    ...
    <form id="search-form" class="dh-search-form-v2">
        <div class="dh-route-field">
            <div class="dh-route-half">
                <label for="origin-search">{{ __('Nereden') }}</label>
                <input type="text" id="origin-search" class="dh-route-input" placeholder="{{ __('Şehir ya da havalimanı') }}" autocomplete="off" value="{{ $originLabel }}">
                <input type="hidden" id="origin" name="origin_airport_id" value="{{ $originId }}">
                <div id="origin-dropdown" class="dh-autocomplete" hidden></div>
            </div>
            <button type="button" id="swap-airports" class="dh-swap-btn" aria-label="{{ __('Kalkış ve varış noktasını değiştir') }}">
                <i class="ti ti-arrows-exchange" aria-hidden="true"></i>
            </button>
            <div class="dh-route-half">
                <label for="destination-search">{{ __('Nereye') }}</label>
                <input type="text" id="destination-search" class="dh-route-input" placeholder="{{ __('Şehir ya da havalimanı') }}" autocomplete="off" value="{{ $destinationLabel }}">
        ...
        <div class="dh-date-field">
            <label for="departure-date">{{ __('Gidiş') }}</label>
            <input type="text" id="departure-date" name="date" placeholder="{{ __('Tarih seçin') }}" value="{{ $date }}">
        </div>

        <div class="dh-date-field" id="return-date-field" {{ $tripType === 'one_way' ? 'hidden' : '' }}>
            <label for="return-date">{{ __('Dönüş') }}</label>
            <input type="text" id="return-date" name="return_date" placeholder="{{ __('Tarih seçin') }}" value="{{ $returnDate }}">
        </div>

        {{-- data-pax-label: yolcu sayısı JS tarafından güncelleniyor, çevrilmiş
             sözcük oradan okunuyor (JS içinde __() çalışmaz). --}}
        <div class="dh-passenger-wrapper" id="passenger-wrapper" data-pax-label="{{ __('Yolcu') }}">
            <div class="dh-passenger-field" id="passenger-toggle">
                <label>{{ __('Yolcular') }}</label>
                <div class="dh-passenger-value" id="passenger-summary">
                    <span class="dh-pax-main-text">{{ $totalPax }} {{ __('Yolcu') }}</span>
            ...
            <div class="dh-passenger-dropdown" id="passenger-dropdown" hidden>
                <div class="dh-dropdown-header">{{ __('Kabin ve yolcu seçimi') }}</div>
                ...
                <div class="dh-pax-list">
                    <div class="dh-pax-row">
                        <div class="dh-pax-info">
                            <strong>{{ __('Yetişkin') }} <i class="ti ti-info-circle" aria-hidden="true"></i></strong>
                            <span>12 +</span>
                        </div>
                    ...
                    <div class="dh-pax-row">
                        <div class="dh-pax-info">
                            <strong>{{ __('Çocuk') }} <i class="ti ti-info-circle" aria-hidden="true"></i></strong>
                            <span>2 - 12</span>
                        </div>
                        <div class="dh-pax-controls">
                    ...
                    <div class="dh-pax-row">
                        <div class="dh-pax-info">
                            <strong>{{ __('Bebek') }} <i class="ti ti-info-circle" aria-hidden="true"></i></strong>
                            <span>{{ __('2 yaş altı') }}</span>
                        </div>
                        <div class="dh-pax-controls">
                    ...
                    <div class="dh-pax-row">
                        <div class="dh-pax-info">
                            <strong>{{ __('Öğrenci') }} <i class="ti ti-info-circle" aria-hidden="true"></i></strong>
                            <span>12 - 35</span>
                        </div>
                        <div class="dh-pax-controls">
                ...
                <div class="dh-pax-footer">
                    <button type="button" class="dh-pax-cancel" id="pax-cancel">{{ __('Vazgeç') }}</button>
                    <button type="button" class="dh-btn-primary dh-pax-apply" id="pax-apply">{{ __('Uygula') }}</button>
                </div>
        ...
        <div class="dh-form-actions">
            <button type="submit" class="dh-btn-primary dh-search-submit">
                {{ __('Uçuş ara') }} <i class="ti ti-arrow-right" aria-hidden="true"></i>
            </button>
        </div>
    </form>
</div>
...
<div id="destinations-modal" class="dh-modal" hidden>
    ...
    <div class="dh-modal-box">
        <div class="dh-modal-header">
            <span>{{ __('Aşağıdaki ülke ve şehirler arasından seçim yapabilirsiniz.') }}</span>
            <button type="button" id="modal-close" class="dh-modal-close" aria-label="{{ __('Kapat') }}">
        ...
        <div class="dh-modal-body">
            <div class="dh-modal-col">
                <div class="dh-modal-col-title">
                    ...
                    {{ __('Ülke / Bölge') }} (<span id="country-count">0</span>)
                </div>
            ...
            <div class="dh-modal-col">
                <div class="dh-modal-col-title">
                    ...
                    {{ __('Havalimanı') }} (<span id="airport-count">0</span>)
                </div>
